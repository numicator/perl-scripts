...
sub work
{
	...
	my @files = glob("$querry.??");
	system("cp db/$db.* /tmp/ramdisk/");
	my %Ids;
	foreach my $querry(@files)
	{
		my($fh, $fname) = tempfile($TmpTmpl, DIR => getcwd(), SUFFIX => '.xml', UNLINK => 1);
		my $timeBlast = time();
		#print STDERR "Lib $db is being copied to RAM disk\n";
		print STDERR "Querry $querry against lib $db blast start\n";
		my $cmd = "blastn -outfmt 5 -max_target_seqs 10000000 -db /tmp/ramdisk/$db -evalue 1e-10 -num_threads 12 -query /tmp/ramdisk/$querry -out $fname";
		#print STDERR "$cmd\n";
		print STDERR "Querry $querry lib $db ERROR: blastn returned non-zero exit code\n" if(system($cmd));
		...
		print STDERR "Lib $db got $#a hits and ".(scalar keys %Ids)." unique hits so far, time: ".sprintf("%d:%02d:%02d\n", $h, $m, $s);
		unlink($fname);
	}
